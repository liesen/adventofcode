...
X, N = max(
    (
        (a, len(set([tuple(v(a, b)) for b in asteroids if not np.array_equal(a, b)])))
        for a in asteroids
    ),
    key=lambda x: x[1],
)
...
bsteroids = sorted(
    [a for a in asteroids if not np.array_equal(a, X)],
...
x, y = list(
    chain.from_iterable(
        zip_longest(
            *[list(g) for k, g in groupby(bsteroids, key=lambda a: tuple(v(X, a)))]
        )
    )
)[200 - 1]
print(x * 100 + y)
